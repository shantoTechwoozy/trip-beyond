...
const StateComponent: React.FC<StateComponentProps> = ({ countryCode = 'IN', selectedState, setSelectedState }) => {
  const data = State.getStatesOfCountry(countryCode).map(state => ({
    value: state.isoCode,
    displayValue: `${state.name} - ${state.isoCode}`
  }));

  return <Dropdown options={data} selectedOption={selectedState} setSelectedOption={setSelectedState} />;
};
